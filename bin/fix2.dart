...
void main() {
  ...
  // 2. Distance Filter
  c = c.replaceFirst(
    RegExp(r"final pickupLatVal =.*?if \(distanceMeters > 15000\) \{.*?return false;\s*\}\s*\}\s*\}", dotAll: true),
  ...
  // 3. FCM push
  c = c.replaceFirst(
    RegExp(r'if \(_waitingForRideId == rideId\) \{\s*debugPrint\([^\n]*\);\s*setState\(\(\) \{\s*_waitingForRideId = null;\s*_waitingForRideData = null;\s*\}\);\s*\}', dotAll: true),
  ...
  // 4. FCM cancel
  c = c.replaceFirst(
    RegExp(r'if \(rideId\.isNotEmpty && _waitingForRideId == rideId\) \{\s*debugPrint\([^\n]*\);\s*if \(mounted\) \{\s*setState\(\(\) \{\s*_waitingForRideId = null;\s*_waitingForRideData = null;\s*\}\);\s*\}\s*\}', dotAll: true),
  ...
  // 5. _showRideRequestScreen assignments
  c = c.replaceAll(
    RegExp(r'if \(mounted\) \{\s*setState\(\(\) \{\s*_waitingForRideId = [^;]+;\s*_waitingForRideData = [^;]+;\s*\}\);\s*\}', dotAll: true),
    r'''if (mounted) {
                setState(() {
  ...
  // 6. cancelInterest
  c = c.replaceFirst(
    RegExp(r'void _cancelInterest\(\) \{\s*if \(_waitingForRideId == null\) return;\s*setState\(\(\) \{\s*_waitingForRideId = null;\s*_waitingForRideData = null;\s*\}\);\s*\}', dotAll: true),
  ...
  // 7. Polling Tick
  c = c.replaceFirst(
    RegExp(r'if \(_waitingForRideId != null\) \{.*?// Step 2: Leave local queued', dotAll: true),
  ...
  // 8. Step 4 Check
  c = c.replaceFirst(
    RegExp(r'if \(!_isShowingRequestScreen &&\s*mounted &&\s*_isOnline &&\s*_waitingForRideId == null &&\s*!_isHandlingAssignment\)', dotAll: true),
  ...
  c = c.replaceFirst(
    RegExp(r'if \(_waitingForRideId == rId\) \{\s*// Already waiting for this ride, skip showing popup again\s*continue;\s*\}', dotAll: true),
  ...
  // 9. Debug log
  c = c.replaceAll(r'_waitingForRideId=$_waitingForRideId', r'waitingCount=${_waitingRides.length}');

  // 10. Banner
  c = c.replaceFirst(
    RegExp(r'if \(_waitingForRideId != null && _waitingForRideData != null\) \.\.\.\[.*?\],(\s*\],)', dotAll: true),
  ...
  print('Done applying dart regex.');
}
